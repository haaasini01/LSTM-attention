...
class RNNLM_Model(nn.Module):
  ...
  def forward(self, input_x, initial_state):
    ...
    outputs = self.add_projection(list_of_tensors)
  ...
  def init_hidden(self):
    ...
    """
    ### YOUR CODE HERE
    h_t = torch.zeros(self.config.batch_size, self.config.hidden_size).to(device)
    c_t = torch.zeros(self.config.batch_size, self.config.hidden_size).to(device)
    init_state = (h_t, c_t)
    ### END YOUR CODE
    return init_state
  def init_hidden_lstm(self):
    ...
    init_state = (init_state[0].to(device), init_state[1].to(device))
    ### END YOUR CODE
    return init_state
...
class LSTMLM_Model(nn.Module):
  ...
  def init_hidden(self):
    ...
    cuda_init_state=[]
    for i in range(self.num_layers):
      cuda_init_state.append((init_state[i][0].to(device),init_state[i][1].to(device)))
...
def generate_text(model, config, vocab,  starting_text='<eos>',
  ...
  for i in range(stop_length):
    ...
    prediction = nn.functional.softmax(last_pred, dim=0) 

    ## if you are using cpu, you do not need to change prediction back to cpu. 
...
def run_epoch(our_model, config, model_optimizer, criterion, data, losses, mode='train', verbose=10):
  ...
  for step, (x, y) in enumerate(ptb_iterator(data, config.batch_size, config.num_steps)):
      ...
      if mode=='train':   
        loss.backward()
        torch.nn.utils.clip_grad_norm_(our_model.parameters(), max_norm=5)
      ...
      if verbose and step % verbose == 0:
        sys.stdout.write('\r{} / {} : pp = {}'.format(step, total_steps, np.exp(np.clip(np.mean(total_loss), -50, 50))))
        sys.stdout.flush()
  if verbose:
      sys.stdout.write('\r')
